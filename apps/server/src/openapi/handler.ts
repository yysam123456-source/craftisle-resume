	...
	if (request.method === "GET" && (request.url.endsWith("/spec.json") || request.url.endsWith("/spec"))) {
		const spec = await openAPIGenerator.generate(openAPIRouter, {
			info: {
				...
				version: appVersion,
				description: "Craftisle Resume API",
				license: { name: "MIT", url: "https://github.com/yysam123456/reactive-resume/blob/main/LICENSE" },
				contact: { name: "Craftisle Team", email: "hello@yysam123456.com", url: "https://yysam123456.com" },
			},
			servers: [{ url: `${env.APP_URL}/api/openapi` }],
			externalDocs: { url: "https://docs.resume.craftisle.com", description: "Craftisle Resume Documentation" },
			commonSchemas: {

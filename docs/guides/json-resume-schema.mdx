...
description: "Learn about the Craftisle Resume JSON Schema, available at /schema.json, and how it aims to become the gold standard for portable resume data."
---
...
Craftisle Resume aims to be the **gold standard in JSON Resume Schemas**. A universal, well-documented, and rigorously typed schema that any service can adopt to enable seamless resume portability across the entire job application ecosystem.

Imagine:

- Exporting your resume from Craftisle Resume as a JSON file
...
The Craftisle Resume JSON Schema is publicly available at:

<Info>[https://resume.craftisle.com/schema.json](https://resume.craftisle.com/schema.json)</Info>

This endpoint returns a complete JSON Schema (draft 07 compatible) that describes the structure of a Craftisle Resume document. It includes:
...
```json
{
  "$schema": "https://resume.craftisle.com/schema.json",
  "version": "5.0.0"
}
```

When importing resumes, Craftisle Resume uses this version to handle migrations and ensure backward compatibility.
...
const ajv = new Ajv();
const schema = await fetch("https://resume.craftisle.com/schema.json").then((r) => r.json());
...
- **Job portals** can accept Craftisle Resume JSON files directly
...
Below is the complete JSON Schema for Craftisle Resume. You can also fetch the latest version directly from [https://resume.craftisle.com/schema.json](https://resume.craftisle.com/schema.json).

...
description: "Connect Craftisle Resume to AI tools like Claude Desktop, Cursor, and Codex using the Model Context Protocol"
---

The Craftisle Resume MCP server lets you manage and modify your resumes through any MCP-compatible AI tool — Claude Desktop, Cursor, Codex, and more. It connects to the Craftisle Resume API and exposes tools for listing, reading, and patching resumes using natural language.
...
<Steps>
  <Step title="Choose your authentication method">
    Craftisle Resume MCP supports two authentication methods:
  ...
  <Step title="If using API key, create one">
    Head over to [https://resume.craftisle.com](https://resume.craftisle.com) (or your self-hosted instance), sign in, and navigate to **Settings → API Keys**. Click **Create a new API key**, give it a name, and copy the secret — it's only shown once.
...
```json
{
  "mcpServers": {
    "reactive-resume": {
      "url": "https://resume.craftisle.com/mcp"
    }
  }
}
```
...
```json
{
  "mcpServers": {
    "reactive-resume": {
      "command": "npx",
      "args": ["mcp-remote", "https://resume.craftisle.com/mcp", "--header", "x-api-key:your-api-key"]
    }
  }
}
```
...
## Authentication Details (How Craftisle Resume MCP Works)

Craftisle Resume MCP accepts authentication in this order:
...
Craftisle Resume is configured as an OAuth authorization server for MCP clients:

- The MCP endpoint is `https://resume.craftisle.com/mcp`.
...
```bash
codex mcp add reactive-resume --url https://resume.craftisle.com/mcp
```
...
Add `https://resume.craftisle.com/mcp` as a custom remote MCP connector, then connect with OAuth in Claude's connector UI.
...
If you're running a self-hosted Craftisle Resume instance, replace `https://resume.craftisle.com/mcp` with your instance URL:
...
| "API error (403)"                                             | The resume is locked. Unlock it in the Craftisle Resume dashboard                                                               |

...
description: "A comprehensive guide to self-host Craftisle Resume with Docker (Postgres only), including a detailed environment variable reference and troubleshooting tips."
---
...
Craftisle Resume can be self-hosted using Docker in a matter of minutes, and this guide will walk you through the process. Here are some of the services you'll need to get started:
...
- Docker Hub: `yysam123456/reactive-resume:latest`
- GitHub Container Registry: `ghcr.io/yysam123456/reactive-resume:latest`
...
SMTP_USER=""
SMTP_PASS=""
SMTP_FROM="Craftisle Resume <noreply@resume.craftisle.com>"
SMTP_SECURE="false"
...
# The rest of Craftisle Resume can run without these.
REDIS_URL=""
# Generated using `openssl rand -hex 32`
ENCRYPTION_SECRET=""
...
```
  ...
  <Step title="Create compose.yml">
    This setup runs Postgres and Craftisle Resume on a private Docker network.
...
services:
  ...
  reactive-resume:
    image: yysam123456/reactive-resume:latest
    # image: ghcr.io/yysam123456/reactive-resume:latest
    restart: unless-stopped
    ports:
      - "3000:3000"
    env_file:
...
```
    ...
    <Tip>
      Prefer pulling from Docker Hub? Keep <code>yysam123456/reactive-resume:latest</code>. Prefer GHCR? Swap it to <code>ghcr.io/yysam123456/reactive-resume:latest</code>.
    </Tip>

    <Note>
      In Docker, the Craftisle Resume server listens on <code>PORT</code> and serves both the API and the built web app.
...
```
    ...
    Craftisle Resume should now be available at your `APP_URL` (for the example above: `http://localhost:3000`).
...
```
  ...
  <Accordion title="Email (SMTP, optional)">
    ...
    - **`SMTP_FROM`**: Default from address (for example, `Craftisle Resume <noreply@resume.craftisle.com>`).
...
<Accordion title="AI features (optional)">
  Saved AI provider management is usable only when **`ENCRYPTION_SECRET`** is configured. The AI Agent workspace also requires **`REDIS_URL`**. The rest of Craftisle Resume can run without them.
...
To update your Craftisle Resume installation to the latest available version, follow these steps:
...
Regular backups are essential to protect your data. Craftisle Resume stores data in two places: the PostgreSQL database and file uploads (either local storage or S3).
...
Craftisle Resume exposes a health check endpoint at `/api/health` that verifies the application and its dependencies. It checks **database** and **storage**; if either is unhealthy, the endpoint returns HTTP `503`.
...
This is particularly useful in high-availability setups where you have multiple instances of Craftisle Resume. If one instance becomes unhealthy (for example, it loses database or storage connectivity), the reverse proxy will stop routing traffic to it until it recovers.
...
You can manually verify the health of your Craftisle Resume instance:

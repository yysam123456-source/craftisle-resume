...
Craftisle Resume supports custom OAuth providers, allowing you to integrate with enterprise identity providers and self-hosted authentication solutions. This is particularly useful for organizations that want to:
...
<Info>
  ...
  [resume.craftisle.com](https://resume.craftisle.com), you can use the built-in Google and GitHub sign-in options.
</Info>
...
Craftisle Resume automatically maps user profile data from the OAuth provider. The following fields are used:

| Craftisle Resume Field | OAuth Profile Fields (in order of preference) |
| --------------------- | --------------------------------------------- |
...
```
    ...
    <Warning>
      Use the **plain text** secret in Craftisle Resume's environment, not the hashed version used in Authelia's configuration.
    </Warning>
...
<CardGroup cols={2}>
  <Card title="Use HTTPS" icon="lock">
    Always use HTTPS for both your Craftisle Resume instance and OAuth provider in production. OAuth tokens should never
  ...
  <Card title="Review scopes" icon="list-check">
    Only request the scopes you need. The default (`openid profile email`) is sufficient for Craftisle Resume.
  </Card>
</CardGroup>
